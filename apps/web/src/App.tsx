    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-[9px] font-bold text-slate-400 uppercase italic">
          Tap a block to reveal its identity
        </p>
      </div>
      ...
      <div className="pixel-grid">
        {SCRIPT_SPECIMENS.map((item, idx) => (
          ...
          >
            <span
              className={`text-2xl font-black ${item.font} transition-transform duration-500 group-hover:scale-125`}
            >
              {item.char}
            </span>
            ...
            {activeItem === idx && (
              <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center p-1 animate-in zoom-in-90 duration-200">
                <span className="text-[7px] font-black uppercase tracking-widest text-white mb-1 leading-none text-center">
                  {item.lang}
                </span>
            ...
            <section className="space-y-12">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-4 border-black pb-8">
                <div className="space-y-2">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest text-slate-400">
                    Recently archived specimens from the field
                  </p>
                </div>
                <button
                  onClick={() => setMode(AppMode.CONTRIBUTE)}
                  className="mt-4 md:mt-0 bg-[#cc543a] text-white px-6 py-3 text-[10px] font-black uppercase tracking-widest brutalist-shadow-sm hover:bg-black transition-all"
                >
              ...
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-32 gap-6">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest animate-pulse italic">
                    Connecting to the Archive Matrix...
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 items-start">
                  {allPages.slice(0, 10).map((page, idx) => (
                    <div
                      key={page.id}
                      className={`group bg-white border-2 border-black p-3 brutalist-shadow-sm hover:-translate-y-1 transition-all ${idx % 3 === 0 ? "md:col-span-2 md:row-span-1" : ""}`}
                    >
                      <a href={`#page-${page.id}`} className="block space-y-4">
                        <div className="aspect-square bg-slate-100 border border-black overflow-hidden relative">
                          ...
                          <div className="absolute top-2 left-2 bg-black text-white text-[7px] font-black px-1.5 py-0.5 uppercase tracking-widest">
                            {page.location}
                          </div>
                        </div>
                        <div className="px-1 border-t border-black/5 pt-3">
                          <p className="text-[11px] font-black uppercase truncate text-black leading-none mb-1.5">
                            {page.title}
                          </p>
            ...
            <section className="max-w-5xl pt-20 border-t-8 border-black">
              <div className="space-y-10">
                <p className="serif text-3xl md:text-5xl leading-tight text-slate-900 font-black max-w-4xl tracking-tighter">
                  What started as a personal curiosity for street letterforms
                  has grown into a collaborative digital museum.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
                  <div className="space-y-8">
                    <p className="serif text-2xl leading-relaxed text-slate-700 italic font-bold border-l-8 border-[#cc543a] pl-8 py-2">
                      This is an attempt to archive and create a museum for your
                      captured street letterings, hand-painted signs, unique
                      stencils, and whatever typographic moments made you stop
                      and click.
                    </p>
                  </div>
                  <div className="bg-slate-50 border-4 border-black p-8 brutalist-shadow-lg">
                    <h3 className="text-2xl font-black uppercase tracking-tighter mb-6 border-b-2 border-black pb-4">
                      How It Works
                    </h3>
                    <ol className="space-y-6">
                      {[
                        {
                          icon: Type,
                          text: "Capture unique lettering in the wild",
                        },
                        {
                          icon: MapIcon,
                          text: "Tag it with a location (PIN code)",
                        },
                        {
                          icon: PlusCircle,
                          text: "Upload to the public archive",
                        },
                        {
                          icon: Heart,
                          text: "Preserve urban linguistic heritage",
                        },
                      ].map((step, i) => (
                        <li key={i} className="flex items-start gap-4 group">
                          ...
                          <div className="flex-1 pt-2 text-base font-bold text-slate-800">
                            {step.text}
                          </div>
                        </li>
                      ))}
          ...
          <div className="max-w-3xl mx-auto space-y-12 pb-24">
            ...
            <div className="space-y-8 text-lg leading-relaxed text-slate-700">
              <p className="text-2xl font-bold">
                Preserving disappearing linguistic artifacts.
              </p>
              ...
              <button
                ...
                className="bg-[#cc543a] text-white px-8 py-4 text-sm font-black uppercase tracking-widest brutalist-shadow-sm"
              >
                Back to Gallery
              </button>
            </div>
