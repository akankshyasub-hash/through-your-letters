...
import { MapPin, User, Share2, Trash2, AlertTriangle, AlignLeft } from 'lucide-react';
import { useToastStore } from '../store/useToastStore';
import { API_BASE_URL } from '../constants';

const ZinePage: React.FC<{page: ZinePageData, onDelete?: (id: string | number) => void}> = ({ page, onDelete }) => {
  ...
  return (
    <div id={`page-${page.id}`} className="flex flex-col md:flex-row gap-12 items-start scroll-mt-24 pb-24 border-b-2 border-black/10 last:border-b-0 overflow-hidden">
      <div className="w-full md:w-3/5 relative py-6 group">
        ...
        <div className="p-3 bg-white border-2 border-black brutalist-shadow transition-all duration-500 hover:rotate-1">
          <img src={page.image} className="w-full aspect-square object-cover contrast-125 grayscale hover:grayscale-0 transition-all duration-700" alt={page.title} />
          <div className="p-4 flex justify-between items-center border-t border-black/5 mt-2 bg-slate-50/50">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest">{page.location}</span>
            </div>
            <span className="text-[9px] font-black uppercase text-slate-500">By {page.contributorName}</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-2/5 flex flex-col space-y-8">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            <button onClick={handleShare} className="p-2 border-2 border-black bg-white hover:bg-slate-100" title="Share"><Share2 size={16}/></button>
            <button onClick={handleReport} className="p-2 border-2 border-black bg-white hover:bg-yellow-50 text-yellow-700" title="Report"><AlertTriangle size={16}/></button>
            {onDelete && <button onClick={() => onDelete(page.id)} className="p-2 border-2 border-black bg-white hover:bg-red-600 hover:text-white text-red-600" title="Delete"><Trash2 size={16}/></button>}
          </div>
        </div>
        
        <h2 className="text-5xl font-black tracking-tighter leading-[0.9] drop-shadow-sm break-words">{page.title}</h2>
