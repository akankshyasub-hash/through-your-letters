...
use aws_sdk_s3::{Client, primitives::ByteStream, config::BehaviorVersion, config::Credentials, config::Region};
...
impl R2StorageService {
    pub async fn new(key: String, secret: String, endpoint: String, bucket: String, public_url: String) -> anyhow::Result<Self> {
        ...
        let config = aws_sdk_s3::config::Builder::new()
            ...
            .build();
        Ok(Self { client: Client::from_conf(config), bucket, public_url })
    }
}
...
impl StorageService for R2StorageService {
    ...
    fn get_url(&self, key: &str) -> String { format!("{}/{}", self.public_url, key) }
}